...
RGBTRIPLE kernelx3(int i, int j, int height, int width, RGBTRIPLE image[height][width])
{
    ...
    image[i][j].rgbtRed = acc > 0 ? acc : 0;
    image[i][j].rgbtBlue = acc > 0 ? acc : 0;
    image[i][j].rgbtGreen = acc > 0 ? acc : 0;
    // printf("  %i  ", red_value);
    return image[i][j];
}
